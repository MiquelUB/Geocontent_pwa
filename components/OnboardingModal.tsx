...
import { onboardingStepsCA, type OnboardingStep } from '@/lib/onboarding-content';
...
export function OnboardingModal({ isOpen, onComplete, onSkip }: OnboardingModalProps) {
  ...
  const step = onboardingStepsCA[currentStep];
  const isLastStep = currentStep === onboardingStepsCA.length - 1;
      ...
      <div className="relative w-full max-w-md bg-white dark:bg-gray-900 rounded-2xl shadow-2xl animate-in zoom-in-95 duration-300">
        ...
        <div className="flex items-center justify-between p-4 border-b border-gray-200 dark:border-gray-800">
          <span className="text-sm text-gray-500 dark:text-gray-400">
            {currentStep + 1}/{onboardingStepsCA.length}
          </span>
          <button
            onClick={handleClose}
          ...
          {step.id === 'geolocation' && (
            <div className="p-3 bg-blue-50 dark:bg-blue-900/20 rounded-lg border border-blue-200 dark:border-blue-800">
              <p className="text-sm text-blue-800 dark:text-blue-200">
                💡 <strong>Consell:</strong> Algunes llegendes tenen contingut especial que només es desbloqueja en visitar-les en persona
              </p>
            </div>
          )}
        </div>
