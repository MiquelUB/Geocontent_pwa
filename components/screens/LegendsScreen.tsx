...
export function LegendsScreen({ onNavigate }: LegendsScreenProps) {
  ...
  const categories = [
    { id: "totes", label: "Todas" },
    { id: "criatures", label: "Criaturas" },
    { id: "fantasmes", label: "Fantasmas" },
    { id: "tresors", label: "Tesoros" },
    { id: "màgia", label: "Magia" }
  ];
  ...
  const filteredLegends = legends.filter(legend => {
    ...
    const matchesCategory = selectedCategory === "totes" || 
                           legend.category.toLowerCase() === selectedCategory.toLowerCase();
    ...
    <div className="screen bg-background">
      ...
      <div className="px-4 py-2 bg-muted/50">
        <p className="text-sm text-muted-foreground">
          {filteredLegends.length} {filteredLegends.length === 1 ? 'lugar encontrado' : 'lugares encontrados'}
